...
void collect_one(int data_number)
{
  std::string data_input = current_workspace + "data/closed_chain_calibration/input_data/q_same_dir.txt";
  if (data_number == 2)
  {
    // 0.002
    data_input = current_workspace + "data/closed_chain_calibration/input_data/q_diff_dir.txt";
  }
  else if (data_number == 3)
  {
    // 0.004
    data_input = current_workspace + "data/closed_chain_calibration/input_data/q_same_dir.txt";
  }
#ifdef DEBUG_MODE
  std::cout<<"reading from -- "<<data_input<<std::endl;
#endif
  std::string data_output = current_workspace + "data/closed_chain_calibration/input_data/input_data_" + std::to_string(data_number) + ".txt";
  int input_counter_ = 0; int output_counter_ = 0;
  Eigen::Matrix<double,N_TOT,1> q_1; q_1.setZero();
  std::vector<Eigen::Matrix<double,N_TOT_w_DATA,1>> saver_;
  std::ifstream rf;
  ...
  { 
    ...
    {
      output_counter_++;
      Eigen::Matrix<double,N_TOT_w_DATA,1> saving_q;
...
void for_fixed_calibration(int data_number)
{
  ...
  std::string data_input = current_workspace + "data/fixed_calibration/input_data/input_data_" + arm_name + "_" + std::to_string(data_number) + ".txt";
#ifdef DEBUG_MODE
  std::cout<<"reading from -- "<<data_input<<std::endl;
#endif
  std::string data_output = current_workspace + "data/fixed_calibration/input_data/input_data_" + arm_name + "_" + std::to_string(data_number) + "_sorted.txt";
  int input_counter_ = 0; int output_counter_ = 0;
  Eigen::Matrix<double,7,1> q_1; q_1.setZero();
