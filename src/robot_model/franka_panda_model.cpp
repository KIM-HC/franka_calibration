...
///////////////////////////////////////CHECK ERROR////////////////////////////////////////
Eigen::Vector3d FrankaPandaModel::getTranslation9(const Eigen::VectorXd &q) const
{
  return RigidBodyDynamics::CalcBodyToBaseCoordinates(*model_, q, body_id9_[newDof - 1], ee_position_, true);
}

Eigen::Matrix3d FrankaPandaModel::getRotation9(const Eigen::VectorXd &q) const
{
  return RigidBodyDynamics::CalcBodyWorldOrientation(*model_, q, body_id9_[newDof - 1], true).transpose() * rot_ee_;
}

Eigen::Isometry3d FrankaPandaModel::getTransform9(const Eigen::VectorXd &q) const
{
  Eigen::Isometry3d transform;
  transform.setIdentity();
  transform.linear() = getRotation9(q);
  transform.translation() = getTranslation9(q);

  return transform;
}

void FrankaPandaModel::initModel9(const Eigen::Ref<const Eigen::Matrix<double, 9, 4>> &dh)
{
  model_ = std::make_shared<RigidBodyDynamics::Model>();
  ...
  Eigen::Vector3d global_joint_position[newDof], global_joint_position_new[newDof], axis[newDof];
  Eigen::VectorXd dh_al(9), dh_a(9), dh_d(9), dh_q(9);
  Eigen::Isometry3d transform_joint; transform_joint.setIdentity();
  const Eigen::Ref<const Eigen::VectorXd> &a_offset = dh.col(0);
  const Eigen::Ref<const Eigen::VectorXd> &d_offset = dh.col(1);
  const Eigen::Ref<const Eigen::VectorXd> &q_offset = dh.col(2);
  ...
  global_joint_position[0] = Eigen::Vector3d(0.0, 0.0, 0.0);
  global_joint_position[1] = Eigen::Vector3d(0.0, 0.0, 0.3330);
  global_joint_position[2] = Eigen::Vector3d(0.0, 0.0, 0.3330);
  global_joint_position[3] = Eigen::Vector3d(0.0, 0.0, 0.6490);
  global_joint_position[4] = Eigen::Vector3d(0.0825, 0.0, 0.6490);
  global_joint_position[5] = Eigen::Vector3d(0.0, 0.0, 1.0330);
  global_joint_position[6] = Eigen::Vector3d(0.0, 0.0, 1.0330);
  global_joint_position[7] = Eigen::Vector3d(0.0880, 0.0, 1.0330);
  global_joint_position[8] = Eigen::Vector3d(0.0880, 0.0, 0.926);

  com_position9_[0] = Eigen::Vector3d(0.0, 0.0, 0.0);
  com_position9_[1] = Eigen::Vector3d(0.000096, -0.0346, 0.2575);
  com_position9_[2] = Eigen::Vector3d(0.0002, 0.0344, 0.4094);
  com_position9_[3] = Eigen::Vector3d(0.0334, 0.0266, 0.6076);
  com_position9_[4] = Eigen::Vector3d(0.0331, -0.0266, 0.6914);
  com_position9_[5] = Eigen::Vector3d(0.0013, 0.0423, 0.9243);
  com_position9_[6] = Eigen::Vector3d(0.0421, -0.0103, 1.0482);
  com_position9_[7] = Eigen::Vector3d(0.1, -0.0120, 0.9536);
  com_position9_[8] = Eigen::Vector3d(0.1, -0.0120, 0.8466);
  ...
  rot_ee_ = transform_joint.linear();
  ee_position_ = Eigen::Vector3d(0.0, 0.0, 0.0);
  ee_position_ = rot_ee_ * ee_position_;

  joint_posision9_[0] = global_joint_position_new[0];
  for (int i = 1; i < newDof; i++)
    joint_posision9_[i] = global_joint_position_new[i] - global_joint_position_new[i - 1];

  for (int i = 0; i < newDof; i++)
    com_position9_[i] -= global_joint_position[i];

  RigidBodyDynamics::Math::Vector3d inertia[newDof];
  for (int i = 0; i < newDof; i++)
    inertia[i] = Eigen::Vector3d::Identity() * 0.001;

  for (int i = 0; i < newDof; i++)
  {
    body9_[i] = RigidBodyDynamics::Body(mass[i], com_position9_[i], inertia[i]);
    joint9_[i] = RigidBodyDynamics::Joint(RigidBodyDynamics::JointTypeRevolute, axis[i]);
    if (i == 0)
      body_id9_[i] = model_->AddBody(0, RigidBodyDynamics::Math::Xtrans(joint_posision9_[i]), joint9_[i], body9_[i]);
    else
      body_id9_[i] = model_->AddBody(body_id9_[i - 1], RigidBodyDynamics::Math::Xtrans(joint_posision9_[i]), joint9_[i], body9_[i]);
  }
}